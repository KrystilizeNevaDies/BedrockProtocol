...
import com.nukkitx.protocol.bedrock.v354.BedrockPacketReader_v354;
...
public class BedrockPacketReader_v361 extends BedrockPacketReader_v354 {

    public static final BedrockPacketHelper INSTANCE = new BedrockPacketReader_v361();

    @Override
    protected void registerEntityData() {
        super.registerEntityData();

        this.addEntityData(40, EntityDataType.NPC_DATA);

        this.addEntityData(103, EntityDataType.SKIN_ID);
        this.addEntityData(104, EntityDataType.SPAWNING_FRAMES);
        this.addEntityData(105, EntityDataType.COMMAND_BLOCK_TICK_DELAY);
        this.addEntityData(106, EntityDataType.COMMAND_BLOCK_EXECUTE_ON_FIRST_TICK);
    }
    ...
    public void readEntityData(ByteBuf buffer, EntityDataMap entityDataMap) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            EntityDataType entityDataType = this.entityData.get(metadataInt);
            EntityDataType.ValueType valueType = this.entityDataTypes.get(VarInts.readUnsignedInt(buffer));
            if (entityDataType != null && entityDataType.isFlags()) {
                if (valueType != EntityDataType.ValueType.LONG) {
                    throw new IllegalArgumentException("Expected long value for flags, got " + valueType.name());
                }
                valueType = EntityDataType.ValueType.FLAGS;
            }
            ...
            switch (valueType) {
                ...
                case VECTOR3I:
                    ...
                    break;
                case FLAGS:
                    int index = entityDataType == EntityDataType.FLAGS_2 ? 1 : 0;
                ...
                case VECTOR3F:
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("Unknown entity data valueType received");
            }
            if (entityDataType != null) {
                entityDataMap.put(entityDataType, object);
            } else {
                log.debug("Unknown entity data: {} valueType {} value {}", metadataInt, valueType, object);
            }
        }
    }
    ...
    public void writeEntityData(ByteBuf buffer, EntityDataMap entityDataMap) {
        ...
        for (Map.Entry<EntityDataType, Object> entry : entityDataMap.entrySet()) {
            ...
            EntityDataType.ValueType valueType = EntityDataType.ValueType.from(object);
            VarInts.writeUnsignedInt(buffer, this.entityDataTypes.get(valueType));
            ...
            switch (valueType) {
                ...
                case VECTOR3I:
                    ...
                    break;
                case FLAGS:
                    int flagsIndex = entry.getKey() == EntityDataType.FLAGS_2 ? 1 : 0;
