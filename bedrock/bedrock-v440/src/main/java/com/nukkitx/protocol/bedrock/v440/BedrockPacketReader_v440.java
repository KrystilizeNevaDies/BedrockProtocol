...
import com.nukkitx.protocol.bedrock.v431.BedrockPacketReader_v431;
import io.netty.buffer.ByteBuf;

public class BedrockPacketReader_v440 extends BedrockPacketReader_v431 {
    public static final BedrockPacketReader_v440 INSTANCE = new BedrockPacketReader_v440();

    @Override
    protected void registerEntityData() {
        super.registerEntityData();

        this.addEntityData(120, EntityDataType.BASE_RUNTIME_ID);
        this.addEntityData(121, EntityDataType.FREEZING_EFFECT_STRENGTH);
        this.addEntityData(122, EntityDataType.BUOYANCY_DATA);
        this.addEntityData(123, EntityDataType.GOAT_HORN_COUNT);
        this.addEntityData(124, EntityDataType.UPDATE_PROPERTIES);
    }
    ...
    public void writeGameRule(ByteBuf buffer, GameRuleData<?> gameRule) {
        ...
        int valueType = this.gameRuleTypes.getInt(value.getClass());

        writeString(buffer, gameRule.getName());
        buffer.writeBoolean(gameRule.isEditable());
        VarInts.writeUnsignedInt(buffer, valueType);
        switch (valueType) {
            case 1:
    ...
    public GameRuleData<?> readGameRule(ByteBuf buffer) {
        ...
        int valueType = VarInts.readUnsignedInt(buffer);
        ...
        throw new IllegalStateException("Invalid gamerule valueType received");
    }
